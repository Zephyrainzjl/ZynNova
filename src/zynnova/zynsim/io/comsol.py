...
def write_comsol_mphtxt(
    ...
    *,
    ...
    try:
        with temporary.open("w", encoding="utf-8", newline="") as handle:
            _write_mphtxt(
                ...
                domain_names=normalized_domain_names,
                float_precision=int(float_precision),
                newline=line_ending,
                include_domain_entity_indices=bool(include_domain_entity_indices),
            )
    ...
    return COMSOLMeshExportReport(
        ...
        domain_entity_indices_written=bool(include_domain_entity_indices),
    )
...
def _write_mphtxt(
    ...
    *,
    ...
    float_precision: int,
    newline: str,
    include_domain_entity_indices: bool,
) -> None:
    ...
    # COMSOL ignores comment lines.  These two machine-readable comments make
    # ZynNova's original material IDs reversible after COMSOL's mandatory
    # positive contiguous geometric-entity renumbering.  This is especially
    # important for valid internal phase 0, which otherwise round-trips as 1.
    line(
        "# ZYNNOVA_REGION_ENTITY_MAP "
        + json.dumps(
            {str(int(region)): int(entity) for region, entity in region_entity_map.items()},
            sort_keys=True,
            separators=(",", ":"),
        )
    )
    line(
        "# ZYNNOVA_REGION_NAMES "
        + json.dumps(
            {str(int(region)): str(name) for region, name in domain_names.items()},
            sort_keys=True,
            ensure_ascii=True,
            separators=(",", ":"),
        )
    ...
    domain_entity_count = mesh.n_cells if include_domain_entity_indices else 0
    line(f"{domain_entity_count} # number of geometric entity indices")
