...
## Free-form unstructured Tet4 meshing

ZynMorph now has a geometry-general production path in addition to the voxel
compatibility path.  Arbitrary watertight STL/OBJ/PLY/NPZ surfaces, multiple
material inclusions, nested coatings and true internal voids can be assembled
into one multi-domain PLC and tetrahedralized by the compiled TetGen C++ core.
The outer boundary no longer needs to be a cuboid.  Per-region volume limits,
per-interface triangle-area limits and local refinement zones produce spatially
nonuniform Tet4 cells.

An existing COMSOL Tet4 MPHTXT can also be profiled and used only as a mesh
*density/style* reference on a completely different free-form geometry:

```python
from zynnova.zynmorph import (
    FreeformRegion, SurfaceShell, mesh_freeform_like_reference
)
...
See `docs/FREEFORM_UNSTRUCTURED_MESHING.md` and
